...
const STATUS_FILTERS = ['all', 'reading', 'completed', 'planned', 'dropped', 'on-hold'] as const;
...
function MediaCard({
  ...
  onIncrementChapter: (id: string) => Promise<void>;
    ...
    <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden hover:border-purple-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/10">
      ...
      <div className="p-4">
        ...
        <div className="flex gap-2 mt-4">
          <button
            onClick={() => onIncrementChapter(item.id)}
...
export default function ManhwaPage() {
  ...
  const fetchMedia = useCallback(async () => {
    try {
      const params = new URLSearchParams({ category: 'manhwa' });
      if (activeFilter !== 'all') {
      ...
      showNotification('Failed to load manhwa', 'error');
    } finally {
      setLoading(false);
    }
  ...
  const handleIncrementChapter = async (id: string) => {
    try {
      ...
      if (!res.ok) throw new Error('Failed to update');
      showNotification('Chapter incremented', 'success');
      fetchMedia();
    } catch {
      showNotification('Failed to increment chapter', 'error');
    }
  };
      ...
      <header className="border-b border-gray-800 bg-gray-950/80 backdrop-blur-sm sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Manhwa</h1>
              <p className="text-sm text-gray-400">Track your manhwa reading progress</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 mt-4 text-lg">No manhwa found</p>
            <p className="text-gray-500 mt-1">Add some manhwa to your list to get started</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {media.map((item) => (
