...
const STATUS_FILTERS = ['all', 'listening', 'completed', 'planned', 'dropped', 'on-hold'] as const;
type StatusFilter = typeof STATUS_FILTERS[number];

const STATUS_COLORS: Record<string, string> = {
  listening: 'bg-green-500/20 text-green-400 border-green-500/30',
...
function MediaCard({
  ...
  onIncrementTrack: (id: string) => Promise<void>;
    ...
    <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden hover:border-purple-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/10">
      ...
      <div className="p-4">
        ...
        {item.platforms && (
          <div className="mt-2">
            <span className="text-xs text-gray-500">
              {JSON.parse(item.platforms).join(', ')}
            </span>
          </div>
        )}
        ...
        <div className="flex gap-2 mt-4">
          <button
            onClick={() => onIncrementTrack(item.id)}
...
export default function MusicPage() {
  ...
  const fetchMedia = useCallback(async () => {
    try {
      const params = new URLSearchParams({ category: 'music' });
      if (activeFilter !== 'all') {
      ...
      showNotification('Failed to load music', 'error');
    } finally {
      setLoading(false);
    }
  ...
  const handleIncrementTrack = async (id: string) => {
    try {
      ...
      if (!res.ok) throw new Error('Failed to update');
      showNotification('Track count incremented', 'success');
      fetchMedia();
    } catch {
      showNotification('Failed to increment track', 'error');
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Your music collection and listening history</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 mt-4 text-lg">No music found</p>
            <p className="text-gray-500 mt-1">Add some albums or playlists to your list</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {media.map((item) => (
              <MediaCard
                ...
                onIncrementTrack={handleIncrementTrack}
