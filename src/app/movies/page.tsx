    ...
    <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden hover:border-purple-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/10">
      <div className="relative aspect-[3/4] bg-gray-800">
        ...
        {item.totalEpisodes && item.totalEpisodes > 0 && (
          <div className="absolute bottom-2 left-2 bg-black/70 px-2 py-1 rounded text-xs text-gray-200">
            {item.totalEpisodes} min
          </div>
        )}
      </div>
      ...
      <div className="p-4">
        ...
        <div className="flex gap-2 mt-4">
          <button
            ...
            disabled={updating || isWatched}
            className={`flex-1 px-3 py-1.5 text-white text-xs font-medium rounded-lg transition-colors ${
              isWatched
                ? 'bg-green-600/30 text-green-300 cursor-default'
                : 'bg-purple-600 hover:bg-purple-500 disabled:bg-gray-700 disabled:text-gray-500'
            }`}
          >
...
export default function MoviesPage() {
  ...
  const fetchMedia = useCallback(async () => {
    try {
      const params = new URLSearchParams({ category: 'movie' });
      if (activeFilter !== 'all') {
  ...
  const handleMarkWatched = async (id: string) => {
    try {
      const res = await fetch(`/api/media/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status: 'completed', currentEp: 1 }),
      });
      if (!res.ok) throw new Error('Failed to update');
