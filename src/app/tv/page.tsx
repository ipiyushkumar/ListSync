...
function MediaCard({
  ...
  onIncrementEpisode: (id: string) => Promise<void>;
...
export default function TVPage() {
  ...
  const fetchMedia = useCallback(async () => {
    try {
      const params = new URLSearchParams({ category: 'tv' });
      if (activeFilter !== 'all') {
      ...
      showNotification('Failed to load TV shows', 'error');
    } finally {
      setLoading(false);
    }
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Track your TV series watching progress</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 mt-4 text-lg">No TV shows found</p>
            <p className="text-gray-500 mt-1">Add some TV shows to your list to get started</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {media.map((item) => (
